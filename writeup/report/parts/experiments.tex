...
It can be seen that both HMC algorithms perform well, as does Naïve SGHMC with MH corrections. These 3 are computationally slow when used on large datasets (as already discussed). Naïve SGHMC does not converge to the correct distribution, empirically confirming Corollary 3.1 of \cite{sghmc}. SGHMC is fast and converges, and so it is a successful algorithm. We note here that our diagram for $U(\theta) = -2\theta^2 + \theta^4$ mirrors that in the paper closely.
